...
class AdapterRVFriend(
    ...
    override fun onBindViewHolder(holder: FriendViewHolder, position: Int) {
        ...
        if (currentItem.photo.isEmpty()) {
            holder.ivPictures.setImageDrawable(null)  // Remove image
            holder.ivPictures.visibility = View.GONE  // Hide the ImageView
        } else {
            val photoBtm = AddFriendActivity().stringToBitmap(currentItem.photo)
            photoBtm?.let {
                holder.ivPictures.setImageBitmap(it)
                holder.ivPictures.visibility = View.VISIBLE // Ensure ImageView is visible
            }
        }
    ...
    class FriendViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val ivPictures: ImageView = itemView.findViewById(R.id.iv_pictures)
    }
}
